/* spheresweep_cockpit.pov version 2.1A
 ...
 *       2015-May-10  The aspect ratio is changed to fit the new demo montage.
 ...
 * 1.2   2015-Sep-11  A Bezier sweep is demonstrated (the pilots' arms).
 *       2019-Apr-27  The environmental lighting is changed.
...
#declare C_AMBIENCE = rgb (Rad? 0: <0.075, 0.110, 0.181>);
#default { finish { ambient C_AMBIENCE diffuse 0.75 } }

light_source // Sun
{ <-1, 1, -1> * 100000, rgb <1.298, 1.236, 1.195>
  parallel point_at 0
}
...
{ y, 0
  pigment
  { checker
    rgb <0.160, 0.086, 0.040> // plowed
    rgb <0.137, 0.234, 0.047> // planted
    scale 1000
  }
  #if (!Rad) finish { ambient rgb <0.051, 0.106, 0.304> } #end
}
...
#declare aHat_top = 9;
#declare pv_Hat_top = vrotate (yHat_top * y, aHat_top * x);
...
{ { <0, 0, 0>, <1.2, 0.2, 0>, <0.084, -0.07, 0> },
  { <0, 0.07, 0.0175/1.2>, <1.35, -0.05, 0>, <0, -0.4, -0.25/3> },
  { <0, 0.4, 0.25/3>, <1.35, -1.25, -0.25>, <0, 0, -0.75> },
  { <0, 0, 0>, <1.35, -1.25, -1.75>, <0, 0, 0> },
}
#declare Arm_pts = SSwp_Bezier (Arm_def, "-", 3, -1)
#declare Arm_rs = array[12]
{ 0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.325, 0.3, 0.3, 0.3, 0.3, 0.35
}
#declare Arm = object
{ SphereSweep_Union (SSWP_BEZIER_SPLINE, Arm_pts, Arm_rs, 10)
  translate (YHEAD - 2) * y
}
...
{ pigment
  { gradient y pigment_map
    { [0.27 checker rgb <1, 0.8, 0> rgb <0.85, 0, 0.05>
